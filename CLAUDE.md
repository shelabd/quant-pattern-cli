...
- **scalp.py** — Intraday scalp levels (`qpat scalp`, default SPY). Pure logic: floor/ceiling from three candidate families — nearest-expiry OI walls (`oi_wall_candidates`, band ±2.5%, plus gamma-weighted magnet strike), ATM-IV expected move over the *remaining* session minutes (`remaining_sigma`, clamped to the 390-minute session), and price structure (`intraday_candidates`: VWAP, opening range once 30 min have printed, session + prior-session levels) plus a volume profile (`volume_profile_candidates`: point-of-control + up to 2 high-volume nodes from CLUSTER_PCT-wide bins of today's bars — levels with real acceptance outrank prices gapped through). `relative_volume` compares today's cumulative volume to the average of ~5 prior sessions over the SAME elapsed bars; `rvol_regime` tags ≥1.5× as trend-day tape (fades unreliable — counter-trend setups get an explicit get-run-over note) and ≤0.7× as quiet/range-bound. `_pick_level` greedy-clusters same-side candidates within `CLUSTER_PCT` (0.15%), highest total weight wins, and snaps to an OI wall strike when the cluster has one; the strongest losing cluster *between* spot and the winner surfaces as the near floor/ceiling — intermediate structure (e.g. the session high) price must break before the main level is in play. `is_market_open` gates `--cron` runs (weekday + 09:30-16:00 ET; holidays caught by empty intraday bars). `scalp_setups()` turns the levels into mechanical entry/exit plans (`ScalpSetup`): long buys the floor, short fades the ceiling — mean-reversion between the extremes, the only trade the levels support. Entry is a zone (±`ENTRY_ZONE_PCT` 0.05%, levels are cluster centers not ticks), stop sits `STOP_BUFFER_PCT` (0.10%) beyond the level, T1 = nearest of VWAP/magnet ≥1R inside the range (else mid-range), T2 = just inside the opposite level; a plan with reward:risk to T2 below `MIN_RR` (1.5) carries a `skip_reason` instead of numbers, counter-trend sides (vs spot-VWAP) and T2-beyond-remaining-1σ get warning notes. Snapshots append to `~/.qpat/scalp_journal.jsonl`; launchd agent `com.shelabd.qpat-scalp-spy` fires every 30 min 14:00-21:30 London and lets the ET guard filter (DST-proof). `format_message` renders the Telegram text. **Level-touch watcher** (`qpat scalp-watch`, launchd `com.shelabd.qpat-scalp-watch-spy` every 60s with the same ET guard): reads the latest journaled snapshot (must be from today), fetches one live price (`YFinanceProvider.get_last_price` — fast_info, 1m-bar fallback), and `check_level_hits` fires an *approach* while price is within `APPROACH_PCT` (0.25%) of a level and not moving away from it (the predictive heads-up: distance, pace-based ETA via `approach_eta` from the previous poll stored in the state file's `last` sample, and the full setup to stage), a *touch* when price enters a level's entry zone, or a *break* when it trades beyond the setup's stop (plan invalidated); `filter_new_hits` dedups to one alert per (side, kind, level) per day via `~/.qpat/scalp_watch_state.json` — a moved level re-arms automatically because the price is in the key, and `rearm_approaches` additionally re-arms an approach once price retreats `APPROACH_REARM_PCT` (0.5%) from the level so a second run at it alerts again (touches stay once-per-day: the approach covers retests). Near levels are deliberately unwatched (VWAP is touched constantly). **Event warnings**: the CLI checks `EventCatalog` for today's broad-market events and `event_warnings()` prepends "stand aside into the print" (before), "still digesting" (<1h after), or nothing (>1h after) using `EVENT_PRINT_TIMES_ET` (8:30 for CPI/PPI/NFP/retail/GDP, 14:00 FOMC; unknown categories warn without a clock). **Journal scoring** (`qpat scalp --score`): `simulate_setup` mechanically replays each setup against post-snapshot 5m bars (limit entry at trigger, stop/T1 first-touch, stop wins same-bar ties, EOD mark-to-close, no slippage); `score_scalp_journal` dedups on (ticker, day, side, trigger), holds out incomplete sessions as pending, and buckets results overall / by side / with-vs-counter-trend / by RVOL band / stand-asides (a negative stand-aside avg-R means the MIN_RR gate earns its keep). This scored evidence is the intended gate for ever re-tuning SOURCE_WEIGHTS or the setup geometry.
