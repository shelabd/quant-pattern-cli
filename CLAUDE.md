...
- **butterfly.py** — Pin butterfly engine (`qpat fly`). Pure logic, no Rich/Click: drift detection (5/20 EMA + 3-session momentum), gamma-weighted OI pin scoring with round-number bonus, OI-aware expiry selection, adaptive wing width (5→3→2) against the debit ceiling `width/(min_rr+1)`, event-skip warnings. Network only inside `recommend_fly()`; chains come from yfinance directly (TODO: extend `DataProvider` with options support). Falls back to volume-weighted scoring when a fresh weekly chain reports zero OI. Recommends only — never routes orders.
